...
(* is this used? or use replySCs_of_cross? *)
            ...
            (** sc_with_reply \<noteq> None : rp is in a reply stack **)
             ...
             (* i.e. replyNext reply'  *)
                    apply (rule corres_guard_imp)
                      ...
                      apply (rule corres_split[OF update_sc_replies_update_Head_corres[simplified dc_def]])
                         ...
                         apply (rule corres_symb_exec_r_sr)
                            apply (rule corres_guard_imp)
                              apply (rule corres_split[OF cleanReply_reply_sc_update_corres])
                                apply (rule reply_unlink_tcb_corres[simplified dc_def])
                                apply (rule disjI2, simp)
                               apply wpsimp
                              apply wpsimp
