...
lemma replyUnlink_st_tcb_at':
  "\<lbrace>\<lambda>s. tcb_at' t s \<longrightarrow> (if t'=t then P (P' Inactive) else P (st_tcb_at' P' t' s))\<rbrace>
    replyUnlink r t
   \<lbrace>\<lambda>rv s. P (st_tcb_at' P' t' s)\<rbrace>"
...
lemma replyUnlink_st_tcb_at'_wp:
  "\<lbrace>\<lambda>s. reply_at' rptr s \<longrightarrow> (if tptr=tptr' then test Inactive else st_tcb_at' test tptr s)\<rbrace>
...
apply (intro conjI; clarsimp)
  ...
  by (fastf orce simp: obj_at'_def pred_tcb_at'_def)*)

lemma replyUnlink_st_tcb_at'_Inactive:
  "\<lbrace>st_tcb_at' test tptr and K (test Inactive)\<rbrace>
   replyUnlink rptr tptr
   \<lbrace>\<lambda>_. st_tcb_at' test tptr\<rbrace>"
